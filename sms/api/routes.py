...
from database.repository import (
    SMSReportRepository, SMSConversationRepository, SMSMessageRepository
)
from nlp.extractor import extract_from_sms, ExtractionResult
from services.sms_service import get_sms_service
from services.notification_service import (
    send_confirmation, send_ranger_alert, send_clarification,
...
async def receive_sms(
    ...
    conv_repo = SMSConversationRepository(db)
    ...
    # Check for existing conversation (clarification flow)
    conversation = conv_repo.get_active(from_)

    if conversation:
        # Handle clarification response
        return await _handle_clarification(
            conversation, text, from_, db, conv_repo, report_repo
        )

    # New message - extract incident information
    ...
    if extraction.overall_confidence >= settings.confidence_threshold_high:
        ...
        # Both failed - ask for clarification
        clarify_field = extraction.clarification_field or "incident_type"

        conv_repo.create(
            phone_number=from_,
            original_message=text,
            session_state=f"awaiting_{clarify_field}",
            partial_data=extraction.to_dict()
        )

        send_clarification(from_, clarify_field)
...
async def _handle_clarification(
    conversation, response_text: str, phone_number: str,
    db: Session, conv_repo: SMSConversationRepository,
    report_repo: SMSReportRepository
):
    """Handle response to clarification question"""
    state = conversation.session_state
    partial_data = conversation.partial_data or {}

    # Map numeric responses to values
    if state == "awaiting_species":
        species_map = {
            "1": "elephant", "2": "lion", "3": "leopard",
            "4": "buffalo", "5": "hyena", "6": "other"
        }
        value = species_map.get(response_text.strip(), response_text.strip())
        partial_data["species"] = {"value": value, "confidence": 0.9, "method": "clarification"}

    elif state == "awaiting_incident_type":
        incident_map = {
            "1": "livestock_attack", "2": "crop_damage",
            "3": "dangerous_animal", "4": "property_damage", "5": "sighting"
        }
        value = incident_map.get(response_text.strip(), "sighting")
        partial_data["incident_type"] = {"value": value, "confidence": 0.9, "method": "clarification"}
    ...
    has_species = partial_data.get("species", {}).get("value")
    has_incident = partial_data.get("incident_type", {}).get("value")

    if has_species and has_incident:
        # Create report
        report = await _create_report_from_dict(
            partial_data,
            phone_number,
            ...
            report_repo,
            needs_review=True,
            review_reason="Created via clarification flow"
        )
        ...
        return {
            "status": "received",
            "action": "report_created_clarified",
            "incident_id": report.incident_id
        }

    # Need more info - ask next question
    if not has_incident:
        conv_repo.update(conversation, session_state="awaiting_incident_type", partial_data=partial_data)
        send_clarification(phone_number, "incident_type")
        return {"status": "received", "action": "clarification_continued"}

    if not has_species:
        conv_repo.update(conversation, session_state="awaiting_species", partial_data=partial_data)
        send_clarification(phone_number, "species")
        return {"status": "received", "action": "clarification_continued"}

    # Shouldn't reach here, but just in case
    conv_repo.delete(conversation)
    send_default_response(phone_number)
    return {"status": "received", "action": "clarification_failed"}
